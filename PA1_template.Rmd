...
```{r Code Question 1, echo=FALSE, fig.align='center'}
...
```{r Code Question 2, echo=FALSE, eval=TRUE, message=FALSE, fig.align='center'}
...
As shown in the chart above, **the 5-minute interval which on average across all the days in the dataset, contains the maximum mean number of steps is `r  format(index(int[int==max(int)]), "%H:%M")`**, and the mean number of steps value registered at this time is `r round(largest[[1]], digits=1)`. The code generating these results (embedded in the .Rmd file) is: `format(index(int[int==max(int)]), "%H:%M")` and `round(largest[[1]], digits=1)`, respectively.
  
The code used to generate the previous chart and associated calculations is the following: 

```{r copycode 2, ref.label="Code Question 2", echo=TRUE, message=FALSE, eval=FALSE}
```


### 3 - Imputing Missing Values

The number of missing values is **`r sum(is.na(mon$steps))` observations**, which equals **`r round(mean(is.na(mon$steps))*100, digits=1)` %** of the total number of observations.

By looking at the data, we find that missing observations correspond to whole days rather than specific time intervals withing given days. Specifically, all the missing observations are concentrated on the following `r length(unique(mon[is.na(mon$steps), date]))` days: _`r unique(mon[is.na(mon$steps), date])`_. The rest of the `r length(unique(mon[!is.na(mon$steps), date]))` days in the data have complete observations. 
...
```{r Code Question 3, echo=FALSE, message=FALSE, fig.align='center'}
...
We observe that this histogram differs from the one we drew without including the NAs (note that both are drawn to the same scale). The assignment of eight new days, each with total steps taken equal to `r as.character(sum(interval.steps[, V1]))`, leads to the visible concentration of the distribution of the _total number of steps per day_ around the mean. This is evident not only from looking at the two histograms, but also by looking at the density kernels of both distributions. Note that for ease of comparison, we have added to this plot in red the kernel density of the previous histogram.

The [sample kurtosis][1] of the _total number of steps per day_ increases from `r s.kurtosis(tot.daily.steps$V1)` in the case when the NAs where ignored to `r s.kurtosis(tot.daily.steps2$V1)` with the eight new observations with identical values. 

We can also compare the two cases, of the data with and without the imputed values, by looking at the following boxplot:

```{r Boxplot Question 3, echo=FALSE, message=FALSE, fig.align='center'}
...
```{r copycode 3a, ref.label="Code Question 3", echo=TRUE, message=FALSE, eval=FALSE}
```
  
And the code for drawing the boxplots comparing both cases is here:
```{r copycode 3b, ref.label="Boxplot Question 3", echo=TRUE, message=FALSE, eval=FALSE}
```
...
```{r Code Question 4, echo=FALSE, message=FALSE, fig.align='center'}
...
```{r copycode 4, ref.label="Code Question 4", echo=TRUE, message=FALSE, eval=FALSE}
```
